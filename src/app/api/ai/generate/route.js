...
export async function POST(request) {
  try {
    ...
    const res = await fetch('https://fal.run/fal-ai/flux-pro/v1', {
      ...
      body: JSON.stringify({ prompt: prompt + ', high quality artwork, ultra detailed, no text, no watermarks, no borders', image_size: { width: 1320, height: 610 }, num_inference_steps: 28, guidance_scale: 3.5, output_format: 'jpeg', safety_tolerance: '2' }),
    });
    if (!res.ok) {
