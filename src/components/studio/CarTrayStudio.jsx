  ...
  const expand = async () => {
    ...
    try {
      const inputs = await composeExpandInputs(),
        ...
        expansionPrompt = [
          "Outpaint only the masked missing area around the original image and create one seamless completed scene.",
          ...
          "Preserve the original image exactly as provided and fill only the missing surrounding area.",
          "Do not add any new people under any circumstance.",
          "If people already exist in the original image, keep only those same people and do not duplicate, clone, mirror, repeat, replace, or invent additional people.",
          "Do not add text, letters, words, typography, captions, signs, posters, labels, headlines, watermarks, or UI-like elements.",
          "Do not add logos, badges, symbols, screenshots, frames, borders, panels, banners, advertisements, or graphic layouts.",
          "Do not create collages, grids, repeated tiles, multiple copies of the image, or repeated subjects.",
          "Do not invent new focal subjects or prominent objects unless the user explicitly requests them.",
          "Never copy, tile, mirror, repeat, collage, stack, zoom, or recreate the whole original image in the expanded area.",
          "Match the original perspective, horizon, lighting, colors, texture, depth, environment, and photographic style so the transition is invisible.",
          "The result must look like one natural continuous photograph, not a poster, graphic design, webpage, advertisement, or collage.",
